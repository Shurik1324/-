# Скрипт автоматической сортировки файлов

Python-скрипт для автоматического распределения файлов по категориальным папкам в зависимости от их расширения. Скрипт корректно обрабатывает дубликаты имен и пропускает скрытые файлы.

## Функционал

* **Категоризация по расширениям**: Распределяет файлы по следующим папкам:
  * `Documents` — `.pdf`, `.docx`, `.doc`, `.txt`, `.xlsx`
  * `Images` — `.jpg`, `.jpeg`, `.png`, `.gif`, `.bmp`, `.webp`
  * `Archives` — `.zip`, `.rar`, `.7z`, `.tar`, `.gz`
  * `Applications` — `.exe`, `.msi`, `.dmg`
  * `Others` — файлы с другими расширениями или без них.
* **Защита от перезаписи**: Если в целевой папке уже существует файл с таким именем, скрипт автоматически добавляет цифровой суффикс (например, `file_1.txt`, `file_2.txt`).
* **Безопасность**: Игнорирует подпапки и скрытые файлы (начинающиеся с точки).
* **Логирование**: Выводит в консоль детальный отчет о каждом перемещенном файле и итоговое количество обработанных данных.

## Требования

* Python 3.4 или выше (используется встроенный модуль `pathlib`).
* Не требует установки сторонних библиотек (используются стандартные модули `shutil` и `pathlib`).

## Использование

1. Откройте файл скрипта и укажите путь к целевой директории в блоке `__main__`:
   ```python
   folder_to_sort = r"C:\Users\User\Downloads" # Замените на ваш путь

Обработка ошибок
•	Скрипт проверяет существование указанной директории перед началом работы. Если путь неверный, выполнение прекращается с выводом ошибки.

•	Перемещение каждого отдельного файла обернуто в блок try-except. Если один из файлов заблокирован системой или недоступен для чтения, скрипт выведет предупреждение и продолжит обработку остальных файлов.
